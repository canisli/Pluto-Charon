...
def main():
    ...
    print(f'Analyzing pluto and charon on {entry}')

    path = f'{config.data_folder}/{config.date}/pluto{config.index}.fits'
    output_path = f'{config.output_folder}/{config.date}/locations{config.index}.csv'
    ...
    print(f'Guessing sigma_x2= {PlutoCharonSetupData["sigma_x2"]}')
    print(f'Guessing sigma_y2= {PlutoCharonSetupData["sigma_y2"]}')

    locations = locate_pluto_charon(PlutoCharonSetupData)
    print(f'Wrote locations to {output_path}')
