...
# Assumes that a starlist.csv file has already been created in the same directory
...
while outside:
    i = 0
    fits = sorted([x for x in glob.glob(path + "**/*.fits", recursive=True) if "1x1" not in x])
    print("\nFits files:")
    for f in fits:
        print(f + " [" + str(i) + "]")
        i += 1
    ...
    while inside:
        starlist = Table.read(fits[fn][0:fits[fn].rfind('/')]+"/starlist.csv", format="csv")
        
        print("\nFirst 10 stars:")
        for i in range(10):
            star = IStar(table_row=starlist[i])
            entry = star.star_name + " (" + str(star.x) + ", " + str(star.y) +")"
            print(entry + ((58-len(entry)) * " ") + "[" + str(i) + "]")
        sn = int(input("\nChoose star: "))    
        
        star = IStar(table_row=starlist[sn])
